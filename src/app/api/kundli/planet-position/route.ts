...
export async function POST(request: NextRequest) {
    try {
        ...
        const { datetime, lat, lon, language = 'en', planets } = body;

        const accessToken = await getAccessToken();
        const coordinates = `${lat},${lon}`;

        const apiUrl = new URL('https://api.prokerala.com/v2/astrology/planet-position');
        ...
        if (planets) {
            apiUrl.searchParams.append('planets', planets);
        }
        ...
        if (!response.ok) {
            ...
            console.error('Planet Position API Error:', errorText);
        ...
        console.error('Error in Planet Position API route:', error);
        return NextResponse.json(
