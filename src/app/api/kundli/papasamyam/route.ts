...
export async function POST(request: NextRequest) {
    try {
        ...
        const { datetime, lat, lon, language = 'en' } = body;

        const accessToken = await getAccessToken();
        const coordinates = `${lat},${lon}`;

        const apiUrl = new URL('https://api.prokerala.com/v2/astrology/papasamyam');
        ...
        if (!response.ok) {
            ...
            console.error('Papasamyam API Error:', errorText);
        ...
        console.error('Error in Papasamyam API route:', error);
        return NextResponse.json(
